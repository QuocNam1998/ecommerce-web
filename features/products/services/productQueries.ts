...
import { fallbackProducts } from "./productFixtures";
...
function getFallbackFeaturedProducts(): Product[] {
  return fallbackProducts.slice(0, 3);
}

function getFallbackNewArrivalProducts(): Product[] {
  return fallbackProducts.slice(3);
}
...
export const fetchNewArrivalProducts = cache(async (): Promise<Product[]> => {
  try {
    ...
    return getFallbackNewArrivalProducts();
  }
});
